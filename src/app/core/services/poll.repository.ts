...
export interface VoteSelection { questionId: string; optionId: string; }
...
export class PollRepository {
  ...
  async getPolls(): Promise<Poll[]> {
    ...
    const { data, error } = await this.supabase.client
      .from('surveys').select('*, questions(*, options(*, votes(*)))').order('created_at', { ascending: false });
    if (error) throw error;
    return (data as SurveyRow[] ?? []).map((row) => this.mapPoll(row));
  }

  async getPoll(id: string): Promise<Poll | null> {
    if (!this.supabase.client) return this.findDemoPoll(id);
    const { data, error } = await this.supabase.client
      .from('surveys').select('*, questions(*, options(*, votes(*)))').eq('id', id).maybeSingle();
    if (error) throw error;
  ...
  async submitVotes(surveyId: string, selections: VoteSelection[], voterId: string): Promise<void> {
    ...
    await this.ensureNotVoted(surveyId, voterId);
    const rows = selections.map((selection) => ({
      survey_id: surveyId, question_id: selection.questionId, option_id: selection.optionId, voter_id: voterId,
    }));
    const { error } = await this.supabase.client.from('votes').insert(rows);
    if (error) throw error;
  }

  private async ensureNotVoted(surveyId: string, voterId: string): Promise<void> {
    const { data, error } = await this.supabase.client!.from('votes')
      .select('id').eq('survey_id', surveyId).eq('voter_id', voterId).limit(1);
    if (error) throw error;
    if (data?.length) throw new Error('You have already submitted this survey.');
  }

  subscribeToVotes(surveyId: string, refresh: () => void): RealtimeChannel | null {
    if (!this.supabase.client) return null;
    return this.supabase.client.channel(`survey-votes-${surveyId}`)
      .on('postgres_changes', this.voteChangeFilter(surveyId), refresh).subscribe();
  }
  ...
  private async insertSurvey(input: CreatePollInput): Promise<{ id: string }> {
    const { data, error } = await this.supabase.client!.from('surveys')
  ...
  private async insertQuestions(surveyId: string, questions: CreatePollInput['questions']): Promise<void> {
    for (const [index, question] of questions.entries()) {
      const questionId = await this.insertQuestion(surveyId, question.text, index);
  ...
  private async insertQuestion(surveyId: string, text: string, position: number): Promise<string> {
    const { data, error } = await this.supabase.client!.from('questions')
      .insert({ survey_id: surveyId, text, position }).select('id').single();
    if (error) throw error;
    return data.id;
  }
  ...
  private mapPoll(row: SurveyRow): Poll {
    return {
      ...
      endDate: row.end_date, createdAt: row.created_at, questions: this.mapQuestions(row.questions ?? []),
    };
  }
  ...
  private demoPoll(id: string, title: string, category: string, days: number): Poll {
    return {
      id, title, category, description: 'We want to make it easy for everyone to share preferences and ideas.',
      endDate: new Date(Date.now() + days * 86_400_000).toISOString(), createdAt: new Date().toISOString(),
  ...
  private createDemoQuestions(input: CreatePollInput): PollQuestion[] {
