...
interface SurveyRow { id: string; title: string; category: string; description: string | null; end_date: string | null; created_at: string; questions?: QuestionRow[]; }
...
export class PollRepository {
  ...
  async getPoll(id: string): Promise<Poll | null> {
    if (!this.supabase.client) return this.demoPolls().find((poll) => poll.id === id) ?? null;
    const { data, error } = await this.supabase.client
      .from('surveys')
      .select('*, questions(*, options(*, votes(*)))')
      .eq('id', id)
  ...
  async createPoll(input: CreatePollInput): Promise<Poll> {
    ...
    const { data, error } = await this.supabase.client
      .from('surveys')
      .insert({ title: input.title, category: input.category, description: input.description,
        end_date: input.endDate })
      .select('id, title, category, description, end_date, created_at')
      .single();
    if (error) throw error;
    await this.insertQuestions(data.id, input.questions);
    const created = await this.getPoll(data.id);
    if (!created) throw new Error('Survey could not be loaded after creation.');
    return created;
  }

  async vote(surveyId: string, questionId: string, optionId: string, voterId: string): Promise<void> {
    if (!this.supabase.client) return;
    const { error } = await this.supabase.client.from('votes').insert({
      survey_id: surveyId, question_id: questionId, option_id: optionId, voter_id: voterId,
    });
    if (error) throw error;
  }
  ...
  subscribeToVotes(surveyId: string, refresh: () => void): RealtimeChannel | null {
    ...
    return this.supabase.client
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'votes', filter: `survey_id=eq.${surveyId}` }, refresh)
      .subscribe();
  }
  ...
  private async insertQuestions(surveyId: string, questions: CreatePollInput['questions']): Promise<void> {
    for (const [questionIndex, question] of questions.entries()) {
      const { data, error } = await this.supabase.client!.from('questions')
        .insert({ survey_id: surveyId, text: question.text, position: questionIndex }).select('id').single();
      if (error) throw error;
      const options = question.answers.map((text, index) => ({ question_id: data.id, text, position: index }));
      const result = await this.supabase.client!.from('options').insert(options);
  ...
  private mapPoll(row: SurveyRow): Poll {
    return {
      ...
      description: row.description ?? '', endDate: row.end_date ?? null, createdAt: row.created_at,
      questions: (row.questions ?? []).sort((a, b) => a.position - b.position).map((question) => ({
        id: question.id, text: question.text,
        answers: (question.options ?? []).sort((a, b) => a.position - b.position).map((option) => ({
          id: option.id, text: option.text, votes: option.votes?.length ?? 0,
        })),
      })),
    };
  }
  ...
  private demoPoll(id: string, title: string, category: string, days: number): Poll {
    ...
    return { id, title, category, description: 'We want to create an easy way for everyone to share their preferences and ideas.',
      endDate: end, createdAt: new Date().toISOString(), questions: [
  ...
  private createDemoPoll(input: CreatePollInput): Poll {
    return { id: crypto.randomUUID(), title: input.title, category: input.category,
      description: input.description, endDate: input.endDate, createdAt: new Date().toISOString(),
      questions: input.questions.map((question) => ({ id: crypto.randomUUID(), text: question.text,
